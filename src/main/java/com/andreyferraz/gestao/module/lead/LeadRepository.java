...
public interface LeadRepository extends CrudRepository<Lead, UUID> {
	...
	@Modifying
	@Query("INSERT INTO lead (id, nome, telefone, orcamento_desenvolvimento, orcamento_manutencao_hospedagem, observacoes, updated_at) VALUES (:id, :nome, :telefone, :orcamentoDesenvolvimento, :orcamentoManutencaoHospedagem, :observacoes, STRFTIME('%Y-%m-%d %H:%M:%f', 'now'))")
	void inserir(
			UUID id,
			String nome,
			String telefone,
	...
	@Query("SELECT id, nome, telefone, orcamento_desenvolvimento, orcamento_manutencao_hospedagem, observacoes, updated_at FROM lead ORDER BY updated_at DESC, id DESC")
